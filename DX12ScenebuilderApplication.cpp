...
SceneBuilderApplication::SceneBuilderApplication()
...
void SceneBuilderApplication::InitExtras()
{
	//init pso
	InitBasicPSO();
...
void SceneBuilderApplication::InitBasicPSO()
{
...
void SceneBuilderApplication::Render()
{
...
void SceneBuilderApplication::PreRenderUpdate()
{
		m_maincameracontroller.Update();
	DX12ApplicationManagerBase::PreRenderUpdate();
}
void SceneBuilderApplication::ProcessWindowProcEvent(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	if (uMsg == WM_CHAR)
	{
		...
		if (charpressed == 'c')
		{
			m_allowCameramovement = !m_allowCameramovement;
			//after reallowing  camera control to avoid a bizare mose move reset mouse first move for camera. 
			m_maincameracontroller.ResetMouseFirstMove();
