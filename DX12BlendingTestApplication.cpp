...
BlendingTestApplication::BlendingTestApplication()
{
	...
	m_basicTransperentPlaneEntitysharedPtr = std::make_shared<BasicRenderableEntity>(MODELTYPE::BASIC_PLANE);
...
void BlendingTestApplication::InitExtras()
{
	//init pso
	InitBasicPSO();
...
void BlendingTestApplication::InitBasicPSO()
{
...
void BlendingTestApplication::Render()
{
...
switch (RENDERENTITIESOPTION)
{
	...
	case RENDERENTITIESOPTION_CUSTOMCREATEDSCENE:
	{
		...
		/* {
			...
			m_shadertransformconsts.model = m_basicTransperentPlaneEntitysharedPtr->GetModelMatrix();
			m_shadertransformconsts.mvp = DirectX::XMMatrixMultiply(m_shadertransformconsts.model, vpmat);
			m_primarycmdlist->SetGraphicsRoot32BitConstants(1, sizeof(m_shadertransformconsts) / 4, &m_shadertransformconsts, 0);
			m_primarycmdlist->SetGraphicsRoot32BitConstants(2, sizeof(m_mat) / 4, &m_mat, 0);
			m_basicTransperentPlaneEntitysharedPtr->Render(m_primarycmdlist);
		}*/
		//draw 6 cube models
		...
		//draw cube
		{
			m_mat.colour = m_basicCubeEntitysharedPtrNZ->GetBasicMaterialData().m_albedo;
			m_shadertransformconsts.model = m_basicCubeEntitysharedPtrNZ->GetModelMatrix();
			m_shadertransformconsts.mvp = DirectX::XMMatrixMultiply(m_shadertransformconsts.model, vpmat);
			m_primarycmdlist->SetGraphicsRoot32BitConstants(1, sizeof(m_shadertransformconsts) / 4, &m_shadertransformconsts, 0);
			m_primarycmdlist->SetGraphicsRoot32BitConstants(2, sizeof(m_mat) / 4, &m_mat, 0);
